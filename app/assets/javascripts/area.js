...
function change(v) {
    var str = "0";
    for (i = 0; i < v; i++) {
        str += ("_" + (document.getElementById(from[i]).selectedIndex - 1));
    };

    var ss = document.getElementById(from[v]);
    with (ss) {
        length = 0;
        options[0] = new Option(opt0[v], opt0[v]);
        if (v && document.getElementById(from[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
...
function to_change(v) {
    var str = "0";
    for (i = 0; i < v; i++) {
        str += ("_" + (document.getElementById(to[i]).selectedIndex - 1));
    };
//    alert(document.getElementById(to[v]));
    var ss = document.getElementById(to[v]);
    with (ss) {
        length = 0;
        options[0] = new Option(opt0[v], opt0[v]);
        if (v && document.getElementById(to[v - 1]).selectedIndex > 0 || !v) {
            if (dsy.Exists(str)) {
                ar = dsy.Items[str];
                for (i = 0; i < ar.length; i++) {
                    options[length] = new Option(ar[i], ar[i]);
...
var from = ["dep_province", "dep_city", "dep_county"];//三个select的id
var to = ["des_province", "des_city", "des_county"];//三个select的id
//var s=["pinche[dep_province]", "pinche[dep_city]", "pinche[dep_county]"];
var opt0 = ["", "", ""];//初始值
function _init_area() {  //初始化函数
    for (i = 0; i < from.length - 1; i++) {
        document.getElementById(from[i]).onchange = new Function("change("+(i+1)+")");
    }
    change(0);
}

function _init_to_area() {  //初始化函数
    for (i = 0; i < to.length - 1; i++) {
        document.getElementById(to[i]).onchange = new Function("to_change("+(i+1)+")");
    }
    to_change(0);
}
